...
func CalculateReservedForSnapshotPerStorageClass(
	...
	if vmSnapshot.Spec.Memory {
		if vm.Status.Hardware != nil && vm.Status.Hardware.Memory != nil && vm.Status.Hardware.Memory.Reservation != nil {
			if _, ok := requestedMap[vm.Spec.StorageClass]; !ok {
				requestedMap[vm.Spec.StorageClass] = resource.NewQuantity(0, resource.BinarySI)
			}

			logger.V(4).Info("adding memory size to the total reserved capacity",
				"memory", vm.Status.Hardware.Memory.Reservation,
				"storageClass", vm.Spec.StorageClass)
			requestedMap[vm.Spec.StorageClass].Add(*vm.Status.Hardware.Memory.Reservation)
		}
	}
