...
func AddToManager(ctx *pkgctx.ControllerManagerContext, mgr manager.Manager) error {
	...
	validatingWebhookConfigKey := client.ObjectKey{
		Name: spqutil.ValidatingWebhookConfigName,
	}

	cache, err := pkgmgr.NewCacheForObjectWithObjectKey(mgr,
		&ctx.SyncPeriod,
		controlledType,
		validatingWebhookConfigKey)
	if err != nil {
		return err
	}
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	if req.Name != spqutil.ValidatingWebhookConfigName {
		pkglog.FromContextOrDefault(ctx).Error(nil, "Reconcile called for unexpected object")
